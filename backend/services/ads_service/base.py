...
class LaunchSpec(BaseModel):
    ...
    ad_copy: AdCopy  # named ad_copy: "copy" shadows BaseModel.copy
    config: LaunchConfig
...
class AdsPlatform(ABC):
    ...
    def get_status(self, platform_ids: Dict[str, str]) -> Dict[str, Any]:
        """Fetch effective/review status for the launched ad."""

    @abstractmethod
    def set_status(self, platform_ids: Dict[str, str], status: str) -> None:
        """Set ACTIVE or PAUSED across the campaign/adset/ad."""

    @abstractmethod
    def get_insights(
        self, platform_ids: Dict[str, str], date_preset: str = "last_7d"
    ) -> List[Dict[str, Any]]:
        """Fetch daily performance metrics (impressions, clicks, ctr, spend, actions)."""
